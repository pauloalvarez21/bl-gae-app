...
/// El backend ya no pagina: la respuesta es la lista completa.
const historicoJson = {
  'baloto': [
    {
      'sorteo': 5221,
...
void main() {
  ...
  testWidgets('renders the BALOTO tab with all its draws', (tester) async {
    ...
    expect(find.text('2 sept 2026'), findsOneWidget);
    expect(find.text('29 ago 2026'), findsOneWidget);
  ...
  testWidgets('requests the histórico without pagination params', (
    tester,
  ) async {
    Uri? capturedUri;
    ...
    // The backend removed server-side pagination: no page/limit query.
